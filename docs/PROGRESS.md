...
| --- | --- |
| Runtime/config | Node 24.20.0 syntax checks pass. The new-app scoped suite has 12/12 `node:test` checks passing, including cart storage-failure fallback, seven-language resource completeness, manifest/icon integrity, production credential startup refusal, private database mode, health/readiness separation, session persistence, CSRF, logout, rate limiting, and static-file isolation. A root-wide `node --test` also collected the separate sample suite and reproduced its known 31/32 result; the project test script targets only `test/`. |
