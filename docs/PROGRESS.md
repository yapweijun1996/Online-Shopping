...
| Product management | Private SQLite v2 and seller API/editor create, edit and change active status for MYR products. Base64 PNG/JPEG/WebP input is bounded and decoded into the private database; inactive product and image paths return 404 publicly. Product API contract tests and a desktop browser flow pass. Order snapshots, localized product text, public catalog UI and checkout availability regression remain; AC-03–04 are partial, so the full-item progress count stays at 1/18. |
...
- The new-app CI workflow has not run remotely. No production build, complete browser E2E, migration/restore, secret scan, deployment, release, or rollback verification exists yet. Local seller smoke and sample checks cannot fill those gaps.

**Resume point:** build public catalog/cart against the active-only API, then guest multi-destination checkout with optional same-device phone/address history, and seller order review. Complete remaining seller product browser checks and localized catalog text during E1 verification. Record implementation, verification, and release evidence per AC row as work proceeds.
