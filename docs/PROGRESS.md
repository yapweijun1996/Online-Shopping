...
| --- | --- |
| Runtime/config | Node 24.20.0 syntax checks pass. The new-app scoped suite has 18/18 `node:test` checks passing, including cart storage-failure fallback, seven-language resource completeness, +60/+65 contact and core-product input validation, manifest/icon integrity, production credential startup refusal, private database mode, health/readiness separation, session persistence, CSRF, logout, rate limiting, and static-file isolation. A root-wide `node --test` also collected the separate sample suite and reproduced its known 31/32 result; the project test script targets only `test/`. `npm audit --omit=dev` reported zero vulnerabilities for the one pinned runtime dependency. |
| Browser | Chromium seller login, reload persistence, account/profile, mobile drawer/Escape/focus return, and logout worked at 390px and desktop widths. Both current shells switched through all seven languages at 390px with matching HTML language tags and no measured horizontal overflow or console errors. This is bounded shell evidence, not AC-15/18 verification of the missing commerce flows. |
...
| Product validation | A pure server helper bounds SKU, name, description, category, integer minor-unit price, currency, and active status. It requires a supplied allowed-currency policy and rejects unknown image fields for now. No product persistence/API or seller editor exists, so AC-03–04 remain incomplete. |
