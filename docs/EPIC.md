...
E1 authentication and catalog API precede the public catalog and E2 checkout. E2 durable order records precede E3 review. E4 checks run incrementally but its release gate follows E1–E3. MYR, no shipping charges or delivery-area enforcement, and bounded base64 images are recorded local-MVP choices. The selected two-container deployment uses one Caddy frontend and one Node/SQLite backend; hostname, real-data retention, backup and recovery remain open before E4 production release. See [TASK.md](TASK.md) for executable work and [ROADMAP.md](ROADMAP.md) for milestone gates.
