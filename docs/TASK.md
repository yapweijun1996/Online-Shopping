...
Connect the public catalog/cart to the active-only API; finish remaining seller product browser QA and localized catalog text during E1 verification. Implement guest multi-destination checkout with local optional contact history next. Production hosting, backup and retention remain DEC-04 before real-data release. Do not copy sample credentials, customer access links, logistics workflow, or UI into the new app.
