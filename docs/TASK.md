...
| API-03 | P0 / In progress | API-01–02, DEC-02, DEC-05 | Seller create/edit/activate/deactivate and public active-only list/detail/search/category/image APIs use private schema v2. Authorization, CSRF, migration, duplicate SKU, image and visibility tests pass. Add localized product text and order snapshot regression before AC-03–04 completion. |
...
| WEB-01 | P1 / Verified locally | API-03 | Public catalog search/detail and no-image placeholder use the active-only API. Desktop and 320/390px browser checks covered discovery, search, detail and unavailable behavior; product API tests cover active-only visibility. Full AC-04 waits for checkout rejection of unavailable products. |
| WEB-02 | P1 / In progress | WEB-01 | IndexedDB cart storage and page-memory fallback pass local tests. Browser add/change/remove, same-browser reload, totals and unavailable-line feedback pass. Verify cache-failure checkout before AC-05. |
...
| WEB-07 | P1 / Planned | WEB-03 | Offer typing plus selectable prior buyer/recipient phone and address values on this device, with explicit save choice and clear-history control. Keep contact history outside service-worker caches and public lookups; verify reload, selection, clearing and storage-failure fallback. |
...
Implement guest multi-destination checkout with local optional contact history next. Finish remaining seller product browser QA and localized catalog text during E1 verification. Production hosting, backup and retention remain DEC-04 before real-data release. Do not copy sample credentials, customer access links, logistics workflow, or UI into the new app.
