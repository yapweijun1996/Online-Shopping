...
Browser verification must cover seller setup, guest checkout, receipt and seller review at phone and desktop widths, including loading/empty/error/success states, keyboard access, labels/focus/error messages, safe areas, overflow, and console errors. Both new web surfaces are planned as installable PWAs with an offline app shell. Service workers must bypass private API/session/order responses and never queue checkout or review mutations as though they succeeded. Verify install, offline fallback, and update behavior on target devices. Both surfaces localize their UI into the seven languages in [PWA_I18N.md](PWA_I18N.md); English is the default. See [UI_SPEC.md](UI_SPEC.md) for desktop/mobile layout and previews.
