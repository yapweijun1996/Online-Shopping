...
The new root application now has a Node 24 HTTP entry point, private SQLite schema v2, a persistent seller session, seller product API/editor, responsive seller and public shop catalog/cart, contact validators, and targeted tests. The product API owns MYR prices, active-only public visibility, and authenticated image access. A Node 24 CI workflow is configured but has not run remotely. The root still has no order persistence/API, release workflow, or deployed runtime. The local `sample/` is a separate Node HTTP/SQLite PWA prototype: its Commerce/Logistics/Fulfillment rules are business references, while its in-memory sessions, fictional customer links, credentials, PWA cache, and logistics workflow are not new-app design decisions.
