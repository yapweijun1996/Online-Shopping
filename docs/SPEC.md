...
| AC-03 | Seller can create/edit/activate/deactivate a product with validated SKU, name, price, currency, and availability. Existing order snapshots remain unchanged after edits. | Product API and seller UI tests; snapshot regression. |
| AC-04 | Public product list/detail/search/category responses expose active products only; unavailable products cannot be checked out. | Public API contract and browser discovery tests. |
...
| AC-07 | Each order has one or more destinations with separate recipient name, phone, address, postcode, and assigned items; recipient phone accepts valid +60/+65 numbers independently of buyer phone and delivery address country. Field errors identify the affected destination. | Multi-destination and mixed-country-phone valid/invalid API and browser tests. |
| AC-08 | Server loads current active products, rejects invalid quantities, handles currency according to the decided shop policy, and calculates totals from integer minor-unit prices. Client totals are ignored; no GST calculation is included in the MVP. | Price tampering, unavailable product, no-GST, and boundary tests. |
