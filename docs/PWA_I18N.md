...
- Preserve the IndexedDB boundary: the current cart module keeps product IDs and quantities locally with an in-memory fallback; minimal same-browser receipts are planned. Authoritative prices and orders stay on the server. An installable PWA does not make browser storage permanent.
