...
**Status: MVP implementation in progress.** Seller sign-in/session, MYR product management, public catalog/cart, guest multi-destination checkout, and responsive PWA shells run locally. Seller order review, complete i18n flows, and release operations remain unfinished. The local `sample/` prototype is a separate business-rule reference; its code, UI, database, and demo credentials are not the new application.
...
Both web surfaces have separate local PWA manifests/scopes, versioned public-shell caches, and offline pages. Local checkout requires an API response and gives no false success when a response is lost. Installation across target browsers and seller review offline behavior remain unverified. The UI defaults to English and offers, in order, English, Malay, Mandarin, Vietnamese, Thai, Japanese, and Korean. See the [seller desktop/mobile previews](docs/UI_SPEC.md) and [PWA/i18n requirements](docs/PWA_I18N.md).
