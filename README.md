...
**Status: MVP implementation in progress.** Seller sign-in/session, MYR product management, public catalog/cart UI, and responsive PWA shells run locally. Checkout UI, seller order review, complete i18n flows, and release operations remain unfinished. The local `sample/` prototype is a separate business-rule reference; its code, UI, database, and demo credentials are not the new application.
...
Open `/seller/` for seller login and product management, or `/shop/` for active-product discovery and a same-browser cart. The guest order-create API is live locally; the checkout form is not connected yet. Seller product images may be uploaded as PNG, JPEG, or WebP up to 512 KB; the browser sends base64 to the API, which stores decoded bytes in the private database. The public product API shows only active MYR products and serves their images at active-only URLs. `/health` is liveness; `/ready` checks the local database schema. The default private database path is `.local/online-shopping.db`. Run `npm ci && npm test` with Node 24, or `npx --yes node@24 --test test/cart.test.js test/checkout-input.test.js test/i18n.test.js test/orders.test.js test/phone.test.js test/product-input.test.js test/products.test.js test/pwa.test.js test/server.test.js` on this machine after `npm ci`. The test script deliberately excludes the separate `sample/` suite. The application is for local development only until the production decisions and release gates in [PROGRESS.md](docs/PROGRESS.md) are resolved.
...
Seller login, basic product management, and public catalog/cart now run locally. Next connect the checkout form and build seller order review against the backend API. The initial super admin username and password come from a local, ignored `.env` file. There is no built-in credential in source or committed configuration. Production startup rejects known development defaults and missing or weak credentials.
