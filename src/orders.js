...
export function createOrder(database, idempotencyKey, input) {
  ...
  try {
    const existing = database.prepare(`SELECT i.request_hash, o.order_no, o.currency, o.total_minor, o.submitted_at
      FROM checkout_idempotency i JOIN shop_order o ON o.id = i.order_id WHERE i.key_hash = ?`).get(keyHash);
    if (existing) {
    ...
    const sequence = database.prepare('UPDATE order_sequence SET value = value + 1 WHERE id = 1 RETURNING value').get();
    ...
    for (const [index, delivery] of order.deliveries.entries()) {
      ...
      database.prepare(`INSERT INTO delivery
    ...
    database.prepare(`INSERT INTO order_event
      ...
      VALUES (?, 'SUBMITTED', 'GUEST', NULL, NULL, 'SUBMITTED', NULL, ?)`).run(id, now);
